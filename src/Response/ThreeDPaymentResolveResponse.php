         ...
         return isset($this->decoded['respCode']) && is_string($this->decoded['respCode']) && trans()->has('payment::payment.yapikredi.messages.'.($this->decoded['respCode'] ?? '0')) ?
             ...
             (
                 $this->successful() ?
                     ...
                     (
                         trans()->has('payment::payment.yapikredi.messages.mdStatus.'.($this->decoded['oosResolveMerchantDataResponse']['mdStatus'] ?? '0')) ?
                         trans('payment::payment.yapikredi.messages.mdStatus.'.($this->decoded['oosResolveMerchantDataResponse']['mdStatus'] ?? '0')) :
                         ($this->decoded['oosResolveMerchantDataResponse']['mdErrorMessage'] ?? trans('payment::payment.error.0'))
                     )
             )
         );
     }
